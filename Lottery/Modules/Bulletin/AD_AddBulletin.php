...
<div class="content">	  
	...
	<thead>
		  ...
		  <th class="TableTdTitle"  style="background-color: rgba(0, 100, 0, 0.9); border-bottom: 1px solid #ddd;"><div class="bTitle">新增新公告</div></th>
		  </tr>
	</thead>
	</table>		
	<tbody>
	...
	<td>
		  <table>
		  <th>公告標題名稱:</th>
		  <td><input type="text" name="bTitle" id="bTitle"  required/></td>
		  </table>
	</td>
	...
	<td>
		  <table>
		  <th>公告上架時間:</th>
		  <td><input type="text" name="bStartDate" id="bStartDate"  placeholder="ex: 2021-01-01"  pattern="\d{4}-\d{2}-\d{2}" required/></td>
		  </table>
	</td>
	...
	<td>
		  <table>
		  <th>公告下架時間:</th>
		  <td><input type="text" name="bEndDate" id="bEndDate"  placeholder="ex: 2021-12-31"  pattern="\d{4}-\d{2}-\d{2}" required/></td>
		  </table>
	</td>
	 ...
	 <td>
		  <table>
		  <th>置頂:</th>
		  <td><input type="checkbox" name="bOnTop[]" value="Y" /></td>
		  </table>
	</td>
	 ...
	 <tr><td class="TableTdContext">
	     ...
	     <th>公告本文：</th>
		 <tr></tr>
		 <td><textarea name="bContext" id="bContext" rows="10" cols="250" required></textarea></td>
		 </table>
		 </td>
	</tr>
	 ...
	 <td class="TableTdURL">
	   ...
	   <table>
	      ...
	      <th>外部相關網頁連結-網頁名稱：</th>
		  <td><input type="text" size="100" id="bAddURLName" name="bAddURLName" /></td>
	    
		
		<tr></tr>
		<th>外部相關網頁連結-URL網址：</th>
		<td><input type="text" size="100" id="bAddURL" name="bAddURL"  /></td>
...
<?php 
	    if(isset($_POST["MM_Insert"]) && isset($_POST["bTitle"]) && 
		isset($_POST["bStartDate"]) && isset($_POST["bEndDate"]) && 
		isset($_POST["bContext"]) &&
		...
		if(isset($_POST["bOnTop"])){
			...
			try{
			 ...
			 date_default_timezone_set("Asia/Taipei");
			  ...
			  $filesName = $date.'__SN__'.$_FILES['bAddFileDir']['name'];
              ...
              if ($_FILES['bAddFileDir']['error'] === UPLOAD_ERR_OK){
			   ...
			   //if(strcmp($signUpError,"") == 0){ 
				...
				// upload add file
				if(!isset($_FILES['bAddFileDir']) || ($_FILES['bAddFileDir']['name'] === "")){
				 $filesName = ""; 
				 $BAddFileDir_file = "";
			    }
				
				
			  $result = $db_link->prepare("INSERT INTO  bulletin(bTitle, bStartDate, bEndDate, bContext, bAddFileName, bAddFileDir, bAddURLName, bAddURL, bOnTop)  VALUES(?,?,?,?,?,?,?,?,?)");
			  ...
			  echo "<script>alert('成功新增公告!');</script>";
			}catch(Exception $e){
				 echo "<script>alert('新增失敗: ".GetSQLValueString($e->getMessage(),"string")."');</script>";
			}
		}
